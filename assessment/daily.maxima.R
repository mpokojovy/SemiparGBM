...
daily.maxima <- function(err1, err2, err.typ = c("MSE", "MAPE"), predtimescale){
  ...
  gbm.inf = which(gbm == -Inf | gbm == Inf) #when all NA's, lines 35 and 36 return -Inf
  ...
  ymin = (1.0/1.05)*min(c(gbm, normal), na.rm = T)
  ymax = (1.0*1.05)*max(c(gbm, normal), na.rm = T) 
  ...
  switch (err.typ,
    ...
    file.name = paste("fig/", plotname, ".pdf", sep = "")
    ...
    lines(x.axis[-c(length(x.axis))], normal, type = "b", col = "black",  lty = 2, pch = 5)
    legend("topleft", c("Semiparametric", "Gaussian"), col = c("black", "black"),
           text.col = c("black", "black"), lty = c(1, 2), pch = c(8,5),
